...
requirePermission('admin', 'locations_all');
...
if ($_POST) {
    if (isset($_POST['add_location'])) {
        ...
        if ($name) {
            $result = $db->query("INSERT INTO locations (location_name, address, created_at) VALUES (?, ?, NOW())",
                                [$name, $address]);
            if ($result) {
    ...
    if (isset($_POST['edit_location'])) {
        ...
        if ($id && $name) {
            $result = $db->query("UPDATE locations SET location_name = ?, address = ?, updated_at = NOW() WHERE id = ?",
                                [$name, $address, $id]);
            if ($result) {
    ...
    if (isset($_POST['delete_location'])) {
        $id = $_POST['location_id'];
        if ($id) {
            $result = $db->query("DELETE FROM locations WHERE id = ?", [$id]);
            if ($result) {
...
$locations = $db->fetchAll("SELECT * FROM locations ORDER BY location_name ASC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>SmartHRM - Locations Management</title>
    ...
    <div class="main-content">
        ...
        <div class="top-navbar">
            <div class="d-flex align-items-center">
                ...
                <h5 class="mb-0 ms-2">Locations Management</h5>
            </div>
        ...
        <div class="dashboard-content">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h2"><i class="fas fa-map-marker-alt me-2"></i>Locations Management</h1>
                <div>
            ...
            <div class="card shadow-sm">
                <div class="card-header">
                    <h5 class="mb-0"><i class="fas fa-list me-2"></i>Locations</h5>
                </div>
                <div class="card-body">
                                ...
                                <?php if (empty($locations)): ?>
                                    <tr>
                                        <td colspan="5" class="text-center py-4">
                                            ...
                                            <p class="text-muted">No locations found. Click "Add Location" to get started.</p>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($locations as $location): ?>
                                        <tr>
                                            ...
                                            <td><?php echo date('M d, Y H:i', strtotime($location['created_at'])); ?></td>
                                            <td>
                                                <button class="btn btn-sm btn-primary me-1" onclick="editLocation(<?php echo $location['id']; ?>, '<?php echo htmlspecialchars($location['location_name']); ?>', '<?php echo htmlspecialchars($location['address'] ?? ''); ?>')">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button class="btn btn-sm btn-danger" onclick="deleteLocation(<?php echo $location['id']; ?>, '<?php echo htmlspecialchars($location['location_name']); ?>')">
    ...
    <div class="modal fade" id="deleteLocationModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST">
                    <div class="modal-header">
                        <h5 class="modal-title"><i class="fas fa-trash me-2"></i>Delete Location</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="alert alert-warning">
                            ...
                            This action cannot be undone and may affect employees assigned to this location.
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" name="delete_location" class="btn btn-danger">
